...
- (CGPoint)getPositionForNewTile:(ZSNewTileNode *)tile {
  return CGPointMake(
      tile.column * TILE_SIZE - (BOARD_COLUMNS * TILE_SIZE) / 2 +
          (TILE_SIZE - SQUARE_SIZE) / 2,
      -(SQUARE_SIZE + NEW_TILE_BUFFER) - (BOARD_ROWS * TILE_SIZE) / 2);
}
...
- (ZSTileNode *)removeTileAtColumn:(int)col andRow:(int)row {
  ...
  NSMutableArray *column_array = [tiles objectAtIndex:col];
  if (row >= [column_array count]) return nil;
...
- (ZSNewTileNode *)addNewTile:(ZSNewTileNode *)tile atColumn:(int)col {
  ...
  if (k != nil) return nil;  // there is already a new tile here
