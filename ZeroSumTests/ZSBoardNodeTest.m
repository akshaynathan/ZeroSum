...
- (void)checkPosition:(ZSTileNode *)tile {
  ...
  if ([tile hasActions]) {
    NSString *key = [NSString
        stringWithFormat:@"moveTo(%d, %d)", (int)correct_x, (int)correct_y];
    XCTAssertNotNil([tile actionForKey:key]);
  } else {
    XCTAssert(tile.position.x == correct_x && tile.position.y == correct_y,
...
- (void)testAddTile {
  ...
  XCTAssert(k == t, "addTile should return the added tile.");
  ...
  XCTAssert(p == t, "addTile should return the added tile.");
  XCTAssert(p.row == 2 && p.column == 0,
            "addTile should set the position of the tile.");
  XCTAssert(k.row == 3 && k.column == 0, "addTile should shift other tiles.");
  [self checkPosition:k];
...
- (void)testRemoveTile {
  ...
  XCTAssertNil(t, "removeTile should return nil for tiles that dont exist.");
  ...
  XCTAssert([t isConnected] == NO, "removeTile should disconnect tile.");
}
...
- (void)testAddNewTile {
  ...
  XCTAssertNil(p, "addNewTile should return nil when there is already a tile.");
...
- (void)testNewTileAt {
  ...
  XCTAssert(k == p, "tileAtColumn should return the correct tile.");
...
- (void)testRemoveNewTile {
  ...
  XCTAssert(l == k, "remove should remove the correct newtile.");

  l = [board newTileAtColumn:5];
  XCTAssert(l == f, "removal should not affect other newtiles.");
}

- (void)testGetFreeColumn {
  int k = [board getFreeColumn];
  XCTAssert(k != -1, "getFreeColumn should return free column index.");
  for (int i = 0; i < BOARD_COLUMNS; i++) {
    ZSNewTileNode *k = [ZSNewTileNode nodeWithValue:5];
    [board addNewTile:k atColumn:i];
  }
  k = [board getFreeColumn];
  XCTAssert(k == -1, "getFreeColumn should return -1 when columns are full.");
}
